# Build-1 Terminal first run  
```
Last login: Mon Dec  8 20:38:47 on ttys000
vivienmacnguyen@Mac ~ % ls
Desktop		Downloads	Movies		Pictures
Documents	Library		Music		Public
vivienmacnguyen@Mac ~ % cd Documents
vivienmacnguyen@Mac Documents % ls
BUILD-1-PROJECT		practice-app-forreal
Prac-This		Stephen Float Serve.mov
vivienmacnguyen@Mac Documents % cd BUILD-1-PROJECT
vivienmacnguyen@Mac BUILD-1-PROJECT % ls
BUILD-1.zip		GodotRNApp		package-lock.json
vivienmacnguyen@Mac BUILD-1-PROJECT % cd GodotRNApp
vivienmacnguyen@Mac GodotRNApp % ls
app			icon.svg		project.binary
app.json		icon.svg.import		README.md
assets			ios			Scenes
components		Models			scripts
constants		node_modules		tsconfig.json
eslint.config.js	package-lock.json
hooks			package.json
vivienmacnguyen@Mac GodotRNApp % npm install

up to date, audited 947 packages in 837ms
...
```
```
 Compiling expo-image Pods/libwebp » picture_tools_enc.c
...
```
```
› Compiling expo-image Pods/libwebp » cost_mips_dsp_r2.c
...
```
```
 Compiling expo-image Pods/libwebp » utils.c
...
```
```
› Compiling expo-image Pods/libwebp » upsampling_neon.c
›
```
```
...
```
```
› Compiling expo-image Pods/libwebp » muxread.c
›
```
```
...
```
```
› Compiling expo-image Pods/libwebp » lossless_enc_mips32.c
›
```
```
...
```
```
 Compiling expo-image Pods/libwebp » huffman_encode_utils.c
...
```
```
 Compiling expo-image Pods/libwebp » buffer_dec.c
...
```
```
› Compiling expo-image Pods/libwebp » backward_references_enc.c
›
```
```
...
```
```
 Compiling expo-image Pods/libdav1d » getbits.c
...
```
```
› Compiling expo-image Pods/libdav1d » lib.c
›
```
```
...
```
```
 Compiling Pods/libavif » write.c
...
```
```
 Copying   expo-image SDWebImage/SDWebImage.bundle/PrivacyInfo.xcprivacy ➜ ios/Pods/SDWebImage/WebImage/PrivacyInfo.xcprivacy
...
```
```
 Copying   ExpoSystemUI/ExpoSystemUI_privacy.bundle/PrivacyInfo.xcprivacy ➜ node_modules/expo-system-ui/ios/PrivacyInfo.xcprivacy
...
```
```
› Compiling expo-image Pods/SDWebImage » UIView+WebCacheState.m
›
```
```
...
```
```
 Compiling expo-image Pods/SDWebImage » SDWebImageDownloaderDecryptor.m
...
```
```
› Compiling expo-image Pods/SDWebImage » NSBezierPath+SDRoundedCorners.m
›
```
```
...
```
```
› Compiling react-native-gesture-handler Pods/RNGestureHandler » RNTapHandler.m
›
```
```
...
```
```
 Compiling Pods/Pods-GodotRNApp » Pods-GodotRNApp-dummy.m
...
```
```
 Packaging Pods/Pods-GodotRNApp » libPods-GodotRNApp.a
...
```
```
 Executing GodotRNApp » [CP] Check Pods Manifest.lock
...
```
```
 Linking   GodotRNApp » __preview.dylib
...
```
```
 Copying   ./PrivacyInfo.xcprivacy ➜ ios/GodotRNApp/PrivacyInfo.xcprivacy
...
```
```
 Copying   ios/GodotRNApp/Supporting/Expo.plist ➜ ./Expo.plist

```
```
› Compiling GodotRNApp » GodotRNApp_vers.c
...
✔ Installed CocoaPods
CommandError: No iOS devices available in Simulator.a  
